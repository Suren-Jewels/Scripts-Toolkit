...
# Multi-cloud load balancing deployment:
# - AWS ALB / NLB
# - Azure Application Gateway / Load Balancer
# - GCP Global HTTP LB / Regional TCP LB
...
AWS_DIR="${ROOT_DIR}/aws/load-balancing"
AZURE_DIR="${ROOT_DIR}/azure/load-balancing"
GCP_DIR="${ROOT_DIR}/gcp/load-balancing"
...
echo "=== AWS Load Balancing: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure Load Balancing: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP Load Balancing: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud load balancing ${ACTION} completed for environment: ${ENVIRONMENT}"
