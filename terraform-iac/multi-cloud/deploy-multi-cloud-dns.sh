...
AWS_DIR="${ROOT_DIR}/aws/dns"
AZURE_DIR="${ROOT_DIR}/azure/dns"
GCP_DIR="${ROOT_DIR}/gcp/dns"
...
echo "=== AWS DNS: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure DNS: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP DNS: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud DNS ${ACTION} completed for environment: ${ENVIRONMENT}"
