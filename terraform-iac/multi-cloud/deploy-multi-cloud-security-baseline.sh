...
# Multi-cloud security baseline deployment:
# - AWS IAM, KMS, GuardDuty, CloudTrail
# - Azure IAM, Key Vault, Defender
# - GCP IAM, KMS, Security Command Center
...
AWS_DIR="${ROOT_DIR}/aws/security"
AZURE_DIR="${ROOT_DIR}/azure/security"
GCP_DIR="${ROOT_DIR}/gcp/security"
...
echo "=== AWS security baseline: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure security baseline: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP security baseline: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud security baseline ${ACTION} completed for environment: ${ENVIRONMENT}"
