...
AWS_DIR="${ROOT_DIR}/aws/kubernetes"
AZURE_DIR="${ROOT_DIR}/azure/kubernetes"
GCP_DIR="${ROOT_DIR}/gcp/kubernetes"
...
echo "=== AWS EKS: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure AKS: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP GKE: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud Kubernetes ${ACTION} completed for environment: ${ENVIRONMENT}"
