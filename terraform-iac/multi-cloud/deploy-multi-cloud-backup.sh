...
AWS_DIR="${ROOT_DIR}/aws/backup"
AZURE_DIR="${ROOT_DIR}/azure/backup"
GCP_DIR="${ROOT_DIR}/gcp/backup"
...
echo "=== AWS Backup: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure Backup: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP Backup: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud backup ${ACTION} completed for environment: ${ENVIRONMENT}"
