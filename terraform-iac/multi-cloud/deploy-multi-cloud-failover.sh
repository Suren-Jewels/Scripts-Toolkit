...
# Multi-cloud failover deployment:
# - AWS Route53 health checks + failover routing
# - Azure Traffic Manager
# - GCP Cloud DNS failover policies
...
AWS_DIR="${ROOT_DIR}/aws/failover"
AZURE_DIR="${ROOT_DIR}/azure/failover"
GCP_DIR="${ROOT_DIR}/gcp/failover"
...
echo "=== AWS Failover: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure Failover: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP Failover: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud failover ${ACTION} completed for environment: ${ENVIRONMENT}"
