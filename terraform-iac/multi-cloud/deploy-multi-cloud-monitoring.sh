...
AWS_DIR="${ROOT_DIR}/aws/monitoring"
AZURE_DIR="${ROOT_DIR}/azure/monitoring"
GCP_DIR="${ROOT_DIR}/gcp/monitoring"
...
echo "=== AWS Monitoring: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure Monitoring: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP Monitoring: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud monitoring ${ACTION} completed for environment: ${ENVIRONMENT}"
