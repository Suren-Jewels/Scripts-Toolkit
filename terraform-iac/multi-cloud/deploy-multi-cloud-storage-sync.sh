...
# Multi-cloud storage sync deployment:
# - AWS S3
# - Azure Blob Storage
# - GCP Cloud Storage
...
AWS_DIR="${ROOT_DIR}/aws/storage-sync"
AZURE_DIR="${ROOT_DIR}/azure/storage-sync"
GCP_DIR="${ROOT_DIR}/gcp/storage-sync"
...
echo "=== AWS Storage Sync: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure Storage Sync: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP Storage Sync: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud storage sync ${ACTION} completed for environment: ${ENVIRONMENT}"
