...
# Multi-cloud networking deployment:
# - AWS VPC + peering/shared services
# - Azure VNet + peering/shared services
# - GCP VPC + shared services
...
AWS_DIR="${ROOT_DIR}/aws/networking"
AZURE_DIR="${ROOT_DIR}/azure/networking"
GCP_DIR="${ROOT_DIR}/gcp/networking"
...
echo "=== AWS networking: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure networking: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP networking: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud networking ${ACTION} completed for environment: ${ENVIRONMENT}"
