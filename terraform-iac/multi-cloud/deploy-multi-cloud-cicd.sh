...
AWS_DIR="${ROOT_DIR}/aws/cicd"
AZURE_DIR="${ROOT_DIR}/azure/cicd"
GCP_DIR="${ROOT_DIR}/gcp/cicd"
...
echo "=== AWS CI/CD: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AWS_DIR}" "${AWS_VARS}" "${ACTION}"

echo "=== Azure CI/CD: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${AZURE_DIR}" "${AZURE_VARS}" "${ACTION}"

echo "=== GCP CI/CD: ${ACTION} (${ENVIRONMENT}) ==="
run_terraform "${GCP_DIR}" "${GCP_VARS}" "${ACTION}"

echo "Multi-cloud CI/CD ${ACTION} completed for environment: ${ENVIRONMENT}"
