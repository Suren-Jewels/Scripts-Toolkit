...
$EndpointsFile = $env:AZURE_UPTIME_ENDPOINTS_FILE
$OutputFile = $env:OUTPUT_FILE
if (-not $OutputFile) { $OutputFile = "azure-uptime.json" }

if (-not $EndpointsFile) {
    Write-Error "AZURE_UPTIME_ENDPOINTS_FILE environment variable is required."
    exit 1
}
...
$final = [PSCustomObject]@{
    meta = @{
        cloud            = "azure"
        collector        = "azure-availability.ps1"
